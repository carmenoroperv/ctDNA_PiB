...
nested_CV_lasso <- function(dataset, k_inner_cv, k_outer_cv, class_type){
    ...
    return_tibble <- foreach(i = 1:k_outer_cv,
        ...
        predicted <- tibble(prob = rep(NA, nrow(dataset)), 
                            pred_class = rep(NA, nrow(dataset)))
        ...
        for (fold in folds){
            ...
            tmp       <- predict(lasso_cva, newx = testdata, s="lambda.min", alpha = best_alpha, type = "response")
            tmp_class       <- predict(lasso_cva, newx = testdata, s="lambda.min", alpha = best_alpha, type = "class")
...
saveRDS(results, file = snakemake@output[["lasso_classification_pred"]])
