...
data <- readRDS("../../data/Meth_and_ATAC_predictions_combined/NEW_CORRECT_PRED/Full_data_ATAC_and_methylation.rds")
...
for (cv_rep in 1:10){
    ...
    for (fold_nr in 1:length(folds)) {
        saveRDS(folds[[fold_nr]], paste("Folds/", "Fold_", fold_nr, "_for_CV_rep_", cv_rep, ".rds", sep = ""))
    }
    }